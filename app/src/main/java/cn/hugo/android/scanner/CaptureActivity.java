...
public final class CaptureActivity extends Activity implements
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			ActivityType = extras.getString("Id");
		}catch (Exception e){
			ActivityType="Error";
		}
...
//                查看是否有该商品
					Cursor cursor = MainActivity.readableDatabase.rawQuery("select quantityofgoods from kucunbiao where usernumber=? and barcode=?", new String[]{MainActivity.UserName, "" + ResultParser.parseResult(rawResult).toString()});
					Toast.makeText(this, "count"+cursor.getCount(), Toast.LENGTH_SHORT).show();
					if(cursor.getCount()!=0) {
//						for (; cursor.moveToFirst(); ) {
						cursor.moveToFirst();
							if (cursor.getInt(0) < 1) {
								new AlertDialog.Builder(CaptureActivity.this).setTitle("数量不足！").setPositiveButton("确定", null).show();
								MainActivity.readableDatabase.endTransaction();
							} else {
//                                减少库存中的商品
								MainActivity.readableDatabase.execSQL("update kucunbiao set quantityofgoods=quantityofgoods-1 where usernumber=? and barcode=?", new Object[]{MainActivity.UserName, "" + ResultParser.parseResult(rawResult).toString()});
//                                获取被减少商品的基本信息
								Cursor cursor1 = MainActivity.readableDatabase.rawQuery("select usernumber,barcode,commodityname,commoditytype,saleprice from kucunbiao where usernumber=? and barcode=?", new String[]{MainActivity.UserName, "" + ResultParser.parseResult(rawResult).toString()});
								cursor1.moveToFirst();
								Date date=new Date();
								SimpleDateFormat simpleDateFormat=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
								String format = simpleDateFormat.format(date);
//                                将基本信息保存到shangpingxingxi表中
                                MainActivity.readableDatabase.execSQL("insert  into `shangpingxingxi`(`usernumber`,`barcode`,`commodityname`,`quantityofgoods`,`commoditytype`,`saleprice`,`adddate`) values (?,?,?,?,?,?,?)",new Object[]{cursor1.getString(0),cursor1.getString(1), cursor1.getString(2), 1, cursor1.getString(3), cursor1.getDouble(4), format});

								MainActivity.datas.add(new ShangPingXingXi(cursor1.getString(0),cursor1.getString(1), cursor1.getString(2), 1, cursor1.getString(3), cursor1.getDouble(4), format));
								One_fragment.OneFragmentAdapter.notifyDataSetChanged();
								MainActivity.readableDatabase.setTransactionSuccessful();
								MainActivity.readableDatabase.endTransaction();
								AlertDialog.Builder builder=new AlertDialog.Builder(CaptureActivity.this).setView(LayoutInflater.from(CaptureActivity.this).inflate(R.layout.add_success_layout,null));
...
//						}
						...
						new AlertDialog.Builder(CaptureActivity.this).setTitle("库存中没有该商品！").setPositiveButton("确定", null).show();
						MainActivity.readableDatabase.endTransaction();
					}
...
//				}
			...
			case "AddActivity":
				AddActivity.et_id_guan_li.setText(ResultParser.parseResult(rawResult).toString());
